...
files=$(printf '%s\n' "$fileshome" "$filescurr" | \
  grep -Ev '\.(bin|class|dat|dds|diff|gpg|gradle|gz|jar|java|js|l|y|mas2j|old|output|[Gg]bx|sh|c|rs|nix|md|lock|o|toml|h|lua|asl|py|json|rasi|html|css|conf|jsonc|log)$' | \
  grep -Ev '^[^\.]*$' | \
